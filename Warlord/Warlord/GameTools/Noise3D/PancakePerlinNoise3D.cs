...
{
    static class PancakePerlinNoise3D
    {
        static public double[,,] GenPancakePerlinNoise3D( PerlinNoiseSettings3D settings )
        {          
            double[,,] noise = new double[settings.size.X, settings.size.Y, settings.size.Z];
            MakePancakePerlinNoise3D( noise, Vector3i.Zero, settings );

            return noise;
        }
        static public double[,,] GenPancakePerlinNoise3D( PerlinNoiseSettings3D settings, int numThreads )
        {            
            double[,,] noise = new double[settings.size.X, settings.size.Y, settings.size.Z];
            ...
            for( int thread = 0; thread < numThreads; thread++ )
            {
                ...
                noiseGenThreads[thread] = new Thread( () => MakePancakePerlinNoise3D( noise, new Vector3i(arrayStartX,0,0), threadSettings ) );
        ...
        static void MakePancakePerlinNoise3D( double[,,] toFill, Vector3i fillStart, PerlinNoiseSettings3D settings )
        {
            int width;
            int height;
            int length;
